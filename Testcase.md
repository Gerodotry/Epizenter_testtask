...
* [5. Тестування безпеки](#5-тестування-безпеки)
...
|----|-------------|-------|----------------------|
| 1 | Користувач не зареєстрований | Відкрити сторінку реєстрації → Ввести валідні дані → Натиснути "Зареєструватись" | Новий акаунт створено, користувач авторизований |
...
| № | Передумови | Кроки | Очікуваний результат |
|----|-------------|-------|----------------------|
| 61 | Форма входу | Ввести SQL-ін’єкцію (`' OR 1=1--`) | Запит не виконується, помилка валідації |
| 62 | Поле пошуку | Ввести JavaScript (`<script>alert('XSS')</script>`) | Код не виконується, фільтрація XSS |
| 63 | HTTPS | Відкрити сайт через http:// | Автоматичне перенаправлення на https:// |
| 64 | Брутфорс логін | Виконати 5 невдалих спроб входу | Користувача тимчасово блокується |
| 65 | Куки | Перевірити прапорці `HttpOnly` та `Secure` | Встановлені для чутливих cookies |
